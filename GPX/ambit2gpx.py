...
""".format(hrext=hrext,tmpext=tmpext,cadext=cadext,wattext=wattext)           
    ...
    def __parse_sample(self, sample, lastdistance, first):
      ...
      </trkpt>""".format(latitude=llatitude, longitude=llongitude, altitude=self.__altitude, time=time, extension=self.extension(self.__hr,self.__temperature,self.__cadence,self.__watts)), file=self.__outputfile)
